...
export function TratamentoAusenciaWizard({ colab, onClose, onSuccess }: Props) {
  ...
  useEffect(() => {
    if (motivo === 'Sem Justificativa') {
        if(sancaoData && sancaoSelecionada === 'Nenhuma') setSancaoSelecionada(sancaoData.sancao_sugerida);
    } else {
      ...
      for (let i = 0; i < diasCobertura; i++) {
        if(!substitutosPorDia[i]){
          ...
          api.getSubstitutos(postoIdToUse, (colab.categoria_cargo || undefined), isoDate, undefined, undefined, (colab.cidade || undefined)).then(data => {
            setSubstitutosPorDia(prev => ({ ...prev, [i]: data.filter((c: any) => c.id !== colab.id) }));
